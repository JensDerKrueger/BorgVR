...
public final class VoiceCommandService: ObservableObject {
            ...
            do {
              ...
              let message = String(
                format: NSLocalizedString(
                  "voice_error_audio_session",
                  comment: "Audio session configuration failed"
                ),
          ...
          case .denied:
            self.transition(
              to: .denied(
                NSLocalizedString(
                  "voice_error_denied",
                  comment: "User denied speech recognition"
                )
              )
            )
          case .restricted:
            self.transition(
              to: .denied(
                NSLocalizedString(
                  "voice_error_restricted",
                  comment: "Speech recognition restricted on this device"
                )
              )
            )
          case .notDetermined:
            self.transition(
              to: .denied(
                NSLocalizedString(
                  "voice_error_not_determined",
                  comment: "Speech authorization not determined"
                )
              )
            )
          @unknown default:
            self.transition(
              to: .denied(
                NSLocalizedString(
                  "voice_error_unknown_auth_state",
                  comment: "Unknown authorization state"
                )
              )
            )
        }
  ...
  public func startListening() {
    ...
    guard let recognizer, recognizer.isAvailable else {
      fail(
        NSLocalizedString(
          "voice_error_unavailable",
          comment: "Speech recognizer unavailable"
        )
      )
      return
    }
    ...
    do {
      ...
      let message = String(
        format: NSLocalizedString(
          "voice_error_start_failed",
          comment: "Start listening failed"
        ),
        error.localizedDescription
      )
      fail(message)
