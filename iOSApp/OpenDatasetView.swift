...
struct OpenDatasetView: View {
  ...
  private func downloadRemoteDatasetBeforeOpening(
    ...
    guard let documentsDirectory = FileManager.default.urls(for: .documentDirectory, in: .userDomainMask).first else {
      throw RemoteDatasetOpenError.documentsDirectoryUnavailable
    }
  ...
  private func deleteDataset(withKey key: String) {
    ...
    let documentsDirectory = FileManager.default.urls(for: .documentDirectory, in: .userDomainMask).first!
    let fileURL = documentsDirectory.appendingPathComponent(dataset.identifier)
    do {
    ...
    return await Task.detached(priority: .userInitiated) {
      ...
      for server in servers {
        do {
          ...
          do {
            let syncedCount = try VolumeMarkerCatalog.storeRemoteMarkerFiles(from: manager, logger: logger)
            if syncedCount > 0 {
              logger.info("Synced \(syncedCount) marker files from \(server.address):\(server.port).")
            }
          } catch {
